...
const SelectDentist = ({ route, navigation }) => {
    const [searchText, setSearchText] = useState('');
    const [selectedDentist, setSelectedDentist] = useState(null); // State to track selected dentist
    const dentists = [
    ...
    const handleDentistSelect = (dentist) => {
        setSelectedDentist(dentist); // Update selected dentist
    };

    const handleSubmit = () => {
        if (selectedDentist) {
            const onSelectDentist = route.params.onSelectDentist;
            onSelectDentist(selectedDentist);
            navigation.goBack();
        }
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView style={styles.subContainer}>
                ...
                {/* Dentists List */}
                {dentists
                    .filter((dentist) => dentist.name.toLowerCase().includes(searchText.toLowerCase()))
                    .map((dentist, index) => (
                        <TouchableOpacity key={index} onPress={() => handleDentistSelect(dentist)}>
                            <View style={styles.card}>
                                <Image source={dentist.image} style={styles.image} />
                                <View style={styles.textContainer}>
                                    <Text style={styles.name}>{dentist.name}</Text>
                                    <Text style={styles.qualification}>{dentist.qualification}</Text>
                                </View>
                                {/* Conditional rendering based on whether the dentist is selected or not */}
                                {selectedDentist && selectedDentist.id === dentist.id ? (
