...
const SelectTeams = ({ route, navigation }) => {
    const [searchText, setSearchText] = useState('');
    const [selectedTeams, setSelectedTeams] = useState(null); // State to track selected dentist
    const teams = [
    ...
    const handleTeamsSelect = (teams) => {
        setSelectedTeams(teams); // Update selected dentist
    };

    const handleSubmit = () => {
        if (selectedTeams) {
            const onSelectTeams = route.params.onSelectTeams;
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView style={styles.subContainer}>
                ...
                {/* Dentists List */}
                {teams
                    .filter((teams) => teams.name.toLowerCase().includes(searchText.toLowerCase()))
                    .map((teams, index) => (
                        <TouchableOpacity key={index} onPress={() => handleTeamsSelect(teams)}>
                            <View style={styles.card}>
                                <Image source={teams.image} style={styles.image} />
                                <View style={styles.textContainer}>
                                    <Text style={styles.name}>{teams.name}</Text>
                                    <Text style={styles.qualification}>{teams.qualification}</Text>
                                </View>
                                {/* Conditional rendering based on whether the dentist is selected or not */}
                                {selectedTeams && selectedTeams.id === teams.id ? (
